...
class AdminModel extends Query {
    ...
    public function getCarpetas($id_usuario){
        $sql = "SELECT * FROM carpetas WHERE id_usuario = $id_usuario AND estado = 1 AND id != 1 ORDER BY id DESC LIMIT 9";
    ...
    {
        $sql = "SELECT id, nombre, apellido, correo, telefono, direccion, clave, rol, usuario, fecha FROM usuarios WHERE id = $id";
        return $this->select($sql);
    }
    ...
    public function subirarchivo($name, $tipo, $id_carpeta, $id_usuario, $tamaño_archivo) {
        $sql = "INSERT INTO archivos (nombre, tipo, id_carpeta, id_usuario, tamaño_archivo) VALUES (?, ?, ?, ?, ?)";
        $datos = array($name, $tipo, $id_carpeta, $id_usuario, $tamaño_archivo);
        return $this->insertar($sql, $datos);
    }

    public function getArchivosRecientes($id_usuario){
        $sql = "SELECT * FROM archivos WHERE id_usuario = 
        $id_usuario AND estado = 1 ORDER BY id DESC LIMIT 10";
        return $this->selectAll($sql);
